...
function ModifyOfferModal({ isOpen, toggleShowModal, poste, posteID }) {
  const dispatch = useDispatch();

  const [formIsValid, setFormIsValid] = useState(false);

  const onePoste = useSelector((state) => state.postes).filter(
    (poste) => poste.id === posteID[0]
  )[0];
  ...
  const [state, setState] = useState({
    ...
    text: onePoste.text,
    totalposts: onePoste.totalposts,
    addedDate: onePoste.addedDate,
    lastModified: onePoste.lastModified
  });
